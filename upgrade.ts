...
import { printWelcome, printRestartHint, abortIfCancelled } from './clack-utils.js';
...
function formatFailingRows(rows: DoctorRow[]): string {
  return rows.map((row) => `  - ${row.label}${row.detail ? `: ${row.detail}` : ''}`).join('\n');
}
...
async function resolveUpgradeMode(options: UpgradeOptions, runtimeExists: boolean): Promise<'upgrade' | 'reinstall'> {
...
export async function runUpgrade(options: UpgradeOptions = {}): Promise<number> {
  ...
  if (!runtimeExists) {
    clack.log.info('No existing runtime detected. Running first-time install flow against this runtime path.');
  } else if (mode === 'reinstall') {
    clack.log.warn('Reinstall will recreate the managed virtualenv but preserve the runtime .env file.');
  }
  ...
  if (failingBefore.length > 0) {
    clack.log.warn(`Existing runtime issues before upgrade:\n${formatFailingRows(failingBefore)}`);
  }
  ...
  if (failingAfter.length === 0) {
    clack.outro(chalk.green(mode === 'reinstall' ? 'blop runtime reinstalled.' : 'blop runtime upgraded.'));
    return 0;
  }

  clack.log.error(`Upgrade finished with remaining issues:\n${formatFailingRows(failingAfter)}`);
  clack.outro(chalk.yellow('Upgrade completed, but some checks still need attention.'));
  return 1;
}
