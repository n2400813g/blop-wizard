...
describe('paths', () => {
  ...
  it('resolves explicit local source path when valid', () => {
    ...
    expect(resolveLocalSourcePath(blopDir)).toBe(path.resolve(blopDir));
  });
  ...
  it('keeps local install runtime separate from local source path', () => {
    const runtimeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'blop-mcp-runtime-test-'),
    );
    expect(
      resolveRuntimePath({ installSource: 'local', runtimePath: runtimeDir }),
    ).toBe(path.resolve(runtimeDir));
  });
  ...
  it('resolves pypi runtime path from explicit arg', () => {
    ...
    expect(
      resolveRuntimePath({ installSource: 'pypi', runtimePath: dir }),
  ...
  it('resolves pypi runtime path from env var', () => {
    ...
    expect(resolveRuntimePath({ installSource: 'pypi' })).toBe(
      path.resolve(dir),
    );
  });
