...
export async function runRepair(options: RepairOptions = {}): Promise<number> {
  ...
  clack.log.warn(`Repairing these issues:\n${formatFailingRows(failingBefore)}`);
  ...
  clack.log.error(`Repair finished with remaining issues:\n${formatFailingRows(failingAfter)}`);
  clack.outro(chalk.yellow('Repair completed, but some checks still need attention.'));
  return 1;
}
